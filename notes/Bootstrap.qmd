...
1.  **Standard errors**: Classical (model-based) standard errors are appropriate when $X$ is fixed. Robust (sandwich) standard errors are designed to protect against misspecification of the variance function when $X$ is random and observed, but they do not improve on classical SEs when $X$ is fixed by design. In Lectures 12 and 13, we used classical SEs for this reason.
