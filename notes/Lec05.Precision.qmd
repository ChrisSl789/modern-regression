...
distributed
   ...
   * $\hat{\theta}$ is our estimate of $\theta$. The $\hat{ }$ indicates it
is an estimate.

   * Mean: $\theta$

* Variance: $V$, which is related to the "average amount of statistical
information" available from each observation
...
* What are the measures of (high) precision?
   ...
   * Estimators are less variable across studies, which is often measured by
decreased standard error.

   * Narrower confidence intervals. Estimators are consistent with fewer
hypotheses if the CIs are narrow.
...
* Translation into sample size
   ...
   * Choose a sample size such that a 95% CI will not contain both the null
...
* Based on statistical power
  ...
  * When the alternative is true, have a high probability of rejecting the
null
...
alternative is true
   ...
   * Most often $\hat{\theta} \sim N\left(\theta, \frac{V}{n} \right)$ so
that the test statistic $Z = \frac{\hat{\theta} - \theta_0}{\sqrt{V/n}}$
wll follow a Normal distribution
...
* Power curves
  ...
  * The power function (power curve) is a function of the true value of
$\theta$

  * We can compute power for every value of $\theta$

  * As $\theta$ moves away from $\theta_0$, power increases (for two-sided
alternatives)

  * For any choice of desired power, there is always some $\theta$ such that
the study has that power
...
* Note that we are not assuming a specific distribution for $Y_i$, just
that the distribution has a mean and variance

* We are assuming that $n$ is large so asymptotic results are applicable

   * Then the distribution $Y_i$ could be binary data, Poisson, exponential,
normal, etc. and the results will hold

* There are ways to decrease $V$ including\...

   * Restrict sample by age, gender, etc.

   * Take repeated measures on each subject, summarize, and perform test on
summary measures
...
* Obsevations no longer identicaly distributed, just independent. Group 1
has a different mean and variance than group 2
...
* If we are constrained by the maximal sample size $n = n_1 + n_2$
   ...
   * In other words, smaller $V$ if we sample more subjects from the more
variable group

* If we are unconstrained by the maximal sample size, there is a point of
diminishing returns

* Example: Case-control study where finding cases is difficult/expensive
but finding controls is easy/cheap

  * Often quoted little benefit beyon $r = 5$.  That is, little benefit in having more than 5 times as many controls as cases
...
* No longer iid. Group 1 has a different mean and variance than group 2,
and observations are paired (correlated)
...
* Clustered data: Experiment where treatments/interventions are assigned
based on the basis of "clusters"
...
* With clustered data, even small correlations can be very important to
consider
...
observation on 1000 different subjects ($n=1000$, $m=1$), or 100 observations on 20
different subjects ($n=20$, $m=100$)?  These two designs have equal precision.
...
(proportions near 0.5)
   ...
   * If we were considering differences in proportions, the max precision is
achieved when the underlying proportions are near 0 or 1
...
* In the PH model, statistical information is roughly proportional to $d$,
the number of observed events

  * Papers always report the number of events

  * Study design must consider how long it will take to observe events (e.g.
deaths) starting from randomization
...
* Precision tends to increases as the predictor ($X$) is measured over a
wider range

* Precision also related to the within group variance $\sigma^2_{Y|X}$

  * What happens to the formulas when $X$ is a binary variable? See two
sample mean
...
* Sample size calculation: The number of sampling units needed to obtain
...
* Sample size required behaves like the square of the width of the CI. To
cut the width of the CI in half, need to quadruple the sample size.

* Positively correlated observations within the same group provide less
precision than the same number of independent observations
...
* What power do you use?
   ...
   * Key is to be able to discriminate between scientifically meaningful
hypotheses
