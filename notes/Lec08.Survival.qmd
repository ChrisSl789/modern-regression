...
* Suppose we assume a Weibull distribution for survival time.  The parameters of interest, $\boldsymbol{\theta}$, typically include:
   ...
   * $\boldsymbol{\beta}$: Regression coefficients for covariates (bold to emphasize their vector nature) to impact the shape parameter of the Weibull distribution
...
* These priors, combined with the likelihood, provide the posterior distribution for inference.
...
- **`stan_surv` in `rstanarm`**:
   ...
   * It is currently (March, 2025) available in the development version of `rstanarm`, which is why it is not demonstrating it.
