...
-   Specifically, as t ranges from 0 to $\infty$, the survival function has the following properties ∗ It is non-increasing ∗ At time t = 0, S(t) = 1. In other words, the probability of surviving past time 0 is 1. ∗ At time $t = \infty$, $S(t) = S(\infty) = 0$. As time goes to infinity, the survival curve goes to 0.
...
-   Scatterplots of censored data are not scientifically meaningful
    ...
    -   Following plot in Stata: "scatter obstime nadir, mlablel(inrem)"

![image](./prophaz/psascatter.pdf)
...
-   Characterization of plot
    ...
    -   First order trends
        ...
        -   No censoring at high nadir PSAs
...
```         
. encode inrem, gen(inrem2)
. gen relapse = abs(inrem2 - 2)
. table relapse inrem
...
-   Define the outcome using "stset"

    -   Time to event (relapse or censored)

    -   Failture indicator (relapse)
...
------------------------------------------------------------------------------
          _t | Haz. Ratio   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
         (1) |   1.166387   .0437864     4.10   0.000     1.083648    1.255443
------------------------------------------------------------------------------
```

-   Interpretation of output

    -   By default, stata gives the hazard ratio, not the coefficient

        -   $\textrm{Hazard ratio} = e^{\textrm{coeff}}$

    -   $\textrm{Hazard ratio} = 1.015^{\Delta nadir}$

    -   Estimated hazard ratio for two groups differing by 1 in nadir PSA is found by exponentiating the slope (Stata only reports the hazard ratio)

        -   Groups 1 unit higher nadir PSA have instantaneous event rate $1.015$ fold higher (1.5% higher)
...
Time at risk         =         1423
                                                   Wald chi2(1)    =     16.79
Log pseudolikelihood =   -113.28918                Prob > chi2     =    0.0000
...
```         
. generate lnnadir = log(nadir)
. stcox lnnadir, robust
...
Time at risk         =         1423
                                                   Wald chi2(1)    =     34.04
Log pseudolikelihood =   -107.31899                Prob > chi2     =    0.0000

------------------------------------------------------------------------------
             |               Robust
          _t | Haz. Ratio   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
     lnnadir |   1.535399   .1128444     5.83   0.000     1.329419    1.773293
------------------------------------------------------------------------------
```

-   Hazard ratio is $1.54$ for a $e$-fold difference in nadir PSA ($e = 2.7183$)
...
-   The confidence interval and statistical test given in the Stata output is called a Wald test. Other tests (Score, Likelihood Ratio) are also possible.
...
-   Log link function
    ...
    | Rates/Means     | $\rightarrow$ Poisson regression                    |     |
    | Medians         | $\rightarrow$ Accelerated Failure Time regression   |     |
...
-   Accelerated Failure Time model
    ...
    -   Basic idea: For every year in a reference group's lives, the other group ages "k" years
        ...
        -   e.g. 1 human year is about 7 dog years
...
-   The following gives the relationship between the hazard function, Survival function, distribution function and cumulative distribution function for four different shape parameters
...
-   Weibull hazard and cumulative hazard at time $t$ with shape $p$ and scale $\lambda$

$$h(t) = λpt^{(p−1)}$$ $$\textrm{where } \lambda = \textrm{exp}(\beta_0 + \beta_1 X_1 + \ldots + \beta_p X_p$$
...
-   A special case of the Weibull model is the Exponential model. The Weibull reduces to the Exponential when the shape parameter is 1 ($p=1$). A shape parameter of 1 assumes the hazard is constant over time, and the cumulative hazard increases linearly with time.
    ...
    -   This provides a method for checking if the Weibull or Exponential model is reasonable for our data. If we estimate the cumulative hazard without making parametric assumptions and plot versus time, we can see if it is increasing linearly or not.
...
-   Cumulative hazard is increasing over time, but not at a constant rate
...
-   How the model is parameterized will either give a proportional hazards or accelerated failure time interpretation (details omitted)
...
-   To obtain the accelerated failure time interpretation, the Weibull model needs to be reparameterized so the output coefficients can be directly interpreted

```{r}
aftreg(formula = Surv(time, status) ~ sex, data = lung,
                         dist = "weibull")
```
