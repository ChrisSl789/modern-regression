...
-   "Dose-response" in the notes that follow more generally refers to "doses" of any type of continuous covariate ("doses of age", "doses of cholesterol", etc.)
...
-   Our scientific interpretation of our statistical tests depends on the meaning of the restricted model compared to the full model
    ...
    -   Consider the additional associations that are possible with the full model that are not possible with the restricted model
...
-   Full model: Survival vs cholesterol and cholesterol$^2$
    ...
    -   Model allows for a parabolic shape for cholesterol
...
Consider the following regression models for outcome $Y$ with continuous predictor $X$ and binary predictor $Z$:

| Model | Formula |
|-------|---------|
| A | $g(\theta) = \beta_0 + \beta_1 X + \beta_2 X^2 + \beta_3 Z + \beta_4 X \cdot Z$ |
| B | $g(\theta) = \beta_0 + \beta_1 X + \beta_2 X^2 + \beta_3 Z$ |
| C | $g(\theta) = \beta_0 + \beta_1 X + \beta_2 X^2$ |
| D | $g(\theta) = \beta_0 + \beta_1 X + \beta_3 Z$ |
| E | $g(\theta) = \beta_0$ |

Which models are nested within which?

::: {.callout-tip collapse="true" title="Click to see answer"}

-   **B is nested in A** (set $\beta_4 = 0$)
-   **C is nested in A** (set $\beta_3 = 0, \beta_4 = 0$)
-   **C is nested in B** (set $\beta_3 = 0$)
-   **D is nested in A** (set $\beta_2 = 0, \beta_4 = 0$)
-   **D is nested in B** (set $\beta_2 = 0$)
-   **E is nested in A, B, C, and D** (set all non-intercept parameters to zero)
-   **C and D are NOT nested** within each other. C contains $\beta_2 X^2$ but not $\beta_3 Z$; D contains $\beta_3 Z$ but not $\beta_2 X^2$. Neither model can be obtained from the other by setting parameters to zero.
...
-   We are often tempted to remove parameters that are not statistically significant before proceeding to other tests
    ...
    -   You will find many textbooks that advocate using this analysis approach
        ...
        -   Such texts are invariably written by mathematicians with no understanding of the underlying science

        -   When you work in the same scientific area, you will eventually "discover" false-positive associations if you use this model building technique
    ...
    -   A procedure where we fit one model, and then use the results to determine the next model we will fit tends to underestimate the true standard error
        ...
        -   A consequence of the general multiple testing problem
...
-   Rather than relying on data-driven model building, we should instead pre-specify how we intend to model our data based on scientific considerations. All models are approximations — the goal is to choose a useful approximation that addresses our scientific question, not to let the data tell us which model to use.
...
-   If we happen to sample over the entire range of $X$ (in this example, from 0 to 100), we would observe all parts of the S-curve
...
-   1:1 Transformations
    ...
    -   Sometimes we can transform 1 scientific measurement into 1 modeled predictor and achieve approximate linearity
...
-   This is the only appropriate method for modeling nominal (unordered) categorical variables
...
-   We can conclude there is a significant difference in salary comparing faculty with different fields ($P<0.001$)

## Modeling Continuous Predictors with Dummy Variables
...
-   Predicted heights for ages 14 to 19 from the dummy variable model

```{r}
newdata[12:17, c("age", "ht.dummy")]
```

### Categorizing a continuous variable

-   Rather than using a dummy variable for each level of age, we could categorize age into broader groups

-   This is a common approach in practice, but it has several drawbacks

```{r, fig.cap="Categorizing age into four groups produces a step function that poorly approximates the true relationship.  Red points are mean height at each year of age."}
fev$age_cat <- cut(fev$age, breaks=c(2, 6, 10, 14, 19),
                   labels=c("3-6", "7-10", "11-14", "15-19"))

m.cat <- lm(height ~ age_cat, data=fev)
newdata$ht.cat <- predict(m.cat, newdata=data.frame(age_cat=cut(3:19, breaks=c(2, 6, 10, 14, 19),
                                                                 labels=c("3-6", "7-10", "11-14", "15-19"))))
ggplot(fev, aes(x=age, y=height)) + geom_point() + theme_bw() +
  geom_line(data=newdata, aes(x=age, y=ht.cat), col="blue", linewidth=1) +
  geom_point(data=newdata, aes(x=age, y=ht.dummy), col="red", size=3)
```

-   Categorizing age into four groups produces a step function

    -   Within each group, the predicted height is the group mean (borrowing strength within groups)

    -   But at the boundaries between groups, there are abrupt jumps in predicted height

    -   The choice of cutpoints (6, 10, 14) is arbitrary — different cutpoints would give different results

    -   Information about the ordering of age within each group is lost (e.g., a 3 year old and a 6 year old are treated the same)

-   We have now seen two extremes

    -   Dummy variable for each year of age: overfit, too many parameters, does not borrow strength across ages

    -   Categorized age: too few parameters, abrupt jumps, arbitrary cutpoints, loses information within groups

-   We need methods that can borrow strength across the range of age while still capturing the non-linear trend — this motivates flexible methods like polynomials and splines
...
-   Piecewise linear curves
    ...
    -   Advantages
        ...
        -   Best to specify knot locations in advance for desired scientific interpretation

        -   Nested within a restricted model with a single slope term (hierarchical structure allows comparisons of complex to simpler models)
...
-   (Intercept) is the expected height when age is 0. Since this is outside the range of our data, it has no scientific interpretation.
...
#### Piecewise linear spline interpretation (marginal is TRUE)
...
-   The third slope term ('lspline(age, knots = c(9.5, 15.5))3') is the change in the age-height slope for 16 to 19 year olds versus 10 to 15 year olds. This output is useful for estimating and testing the slope changes over age.
...
-   This is the same scientific test as for the non-marginal model, just a different parameterization. So, we obtain the same inference (F-stat and p-values are identical).

### Polynomial splines

-   Polynomial splines introduces squared or cubic terms to increase the smoothness of the estimate. In particular, compared to linear splines, there are no abrupt changes in the slope at each knot point.
...
-   A B-spline with no interior knots and degree 3 is mathematically equivalent to a cubic polynomial — the two fits are identical. This shows that B-splines generalize polynomials by adding interior knots.

##### B-spline versus piecewise linear spline
...
-   With the same knot locations, the cubic B-spline produces a smooth curve while the piecewise linear spline has abrupt slope changes at each knot. The B-spline uses more degrees of freedom (5 vs 3) to achieve this smoothness.

##### B-spline versus natural spline

```{r, fig.cap="B-Spline with 3 degrees of freedom and knots at 9.5 and 15.5 compared to ns (3 df).  The enforcement of the linearity constraint beyond the boundary knots is particularly apparent for the natural spline at the low end of age."}
ggplot(fev, aes(x=age, y=height)) + geom_point() + theme_bw() +
...
-   The key difference is at the tails: the natural spline enforces linearity beyond the boundary knots, while the B-spline is unconstrained. This is particularly visible at the low end of age, where the B-spline curves while the natural spline is straight. The linearity constraint makes the natural spline more stable at the extremes and uses fewer degrees of freedom for the same number of interior knots.

##### Natural spline versus restricted cubic spline

-   Natural splines and restricted cubic splines are both cubic splines with restrictions of linearity beyond the boundary knot locations

-   They have different inputs which can result in different model fits

```{r, fig.cap="Natural spline (with knots at 9.5 and 15.5) compared with restricted cubic spline with parms set to give the same fit.  Both spline approaches impose a linearity constraint at the tails.  Fits are same using these inputs."}
ggplot(fev, aes(x=age, y=height)) + geom_point() + theme_bw() +
...
-   When given equivalent knot specifications, `ns()` and `rcs()` produce the same fit. Both enforce linearity at the tails.

```{r, fig.cap="Natural cubic spline (3 d.f.) compared with restricted cubic spline using default inputs.  Both spline approaches impose a linearity constraint at the tails, but with different default knot locations the fits are different."}
ggplot(fev, aes(x=age, y=height)) + geom_point() + theme_bw() +
  geom_smooth(method=lm, formula=y ~ ns(x, df=3), se=FALSE, aes(color="Blue")) +
...
-   However, when using default settings, `ns()` and `rcs()` place knots at different locations, leading to different fits. The choice between `ns()` and `rcs()` matters less than the choice of knot locations.

##### Natural spline versus piecewise linear spline

```{r, fig.cap="Natural spline (3 d.f.) compared with piecewise linear spline with knots at 9.5 and 15.5."}
ggplot(fev, aes(x=age, y=height)) + geom_point() + theme_bw() +
  geom_smooth(method=lm, formula=y ~ ns(x, df=3), se=FALSE, aes(color="Blue")) +
...
-   The natural spline provides a smooth curve while the piecewise linear spline has sharp corners at the knots. The piecewise linear spline has the advantage of interpretable slope coefficients within each segment. The natural spline trades interpretability for smoothness.

##### Key takeaways from the comparisons

-   With equivalent inputs, `ns()` and `rcs()` give the same fit — the choice between them is largely a matter of software preference
-   The linearity constraint at the tails (used by `ns()` and `rcs()` but not `bs()`) improves stability at the extremes and saves degrees of freedom
-   Knot locations matter more than the choice of spline type
-   Piecewise linear splines offer interpretability at the cost of smoothness; polynomial splines offer smoothness at the cost of interpretability

### Comparing models using AIC and BIC

-   When comparing nested models, we can use F-tests or Wald tests to determine if the additional parameters in the full model are necessary

-   However, many of the spline approaches we have considered are not nested within each other, making F-tests and Wald tests inappropriate for comparison

-   Which models are nested?

    -   **Piecewise linear splines** are nested within a model with a single linear term. If all slope-change parameters are zero (using the `marginal=TRUE` parameterization), you recover a straight line

    -   **Polynomials** are nested: a cubic polynomial nests a quadratic which nests a linear term, by setting higher-order coefficients to zero

    -   **Same spline type with different degrees of freedom**: e.g., `ns(x, df=3)` is nested within `ns(x, df=5)` because fewer knots is a restriction of more knots within the same basis family

-   Which models are NOT nested?

    -   **Natural splines (`ns`) vs restricted cubic splines (`rcs`)**: these use different basis functions, so neither is nested within the other even with the same number of knots

    -   **B-splines (`bs`) vs natural splines (`ns`)**: `ns()` adds boundary linearity constraints, but this is a constraint on the basis itself, not a simple "set parameters to zero" relationship

    -   **Piecewise linear splines vs polynomial splines**: these use fundamentally different basis functions

    -   **Categorized (dummy variable) models vs spline models**: a step function is not nested within a smooth curve

-   For comparing non-nested models, we can use information criteria
...
-   Balances model fit ($-2 \log L$) against model complexity ($2p$)

-   Lower AIC indicates a better balance of fit and parsimony

-   Can be used to compare both nested and non-nested models, as long as they are fit to the same data
...
-   Similar to AIC, but with a stronger penalty for model complexity when $n > 7$ (since $\log(n) > 2$)

-   BIC tends to favor simpler models than AIC

-   Lower BIC indicates a better model

#### Example: Comparing models for the height-age relationship

-   We can compare all of the approaches we have considered for modeling the height-age relationship using AIC, BIC, and residual sum of squares (RSS)

-   The RSS is minimized by the saturated model (dummy variable for each age), which fits the data perfectly at each observed age level. However, the saturated model uses the most parameters and is penalized accordingly by AIC and BIC

```{r}
# Fit all models
m.null <- lm(height ~ 1, data=fev)
m.linear <- lm(height ~ age, data=fev)
m.cat4 <- lm(height ~ age_cat, data=fev)
m.poly3 <- lm(height ~ poly(age, 3), data=fev)
m.lspline2 <- lm(height ~ lspline(age, knots=c(9.5, 15.5)), data=fev)
m.ns3 <- lm(height ~ ns(age, df=3), data=fev)
m.rcs5 <- lm(height ~ rcs(age, nk=5), data=fev)
m.saturated <- lm(height ~ factor(age), data=fev)

# Collect results
models <- list(
  "Null (intercept only)" = m.null,
  "Linear" = m.linear,
  "Categorized (4 groups)" = m.cat4,
  "Cubic polynomial" = m.poly3,
  "Linear spline (2 knots)" = m.lspline2,
  "Natural spline (3 df)" = m.ns3,
  "Restricted cubic spline (5 knots)" = m.rcs5,
  "Saturated (factor)" = m.saturated
)
...
-   The saturated model (dummy variable for each year of age) has the smallest RSS because it fits the mean of each age group exactly — but it uses the most parameters (17) and is penalized by both AIC and BIC

-   The categorized model (4 groups) uses few parameters but has a large RSS because the step function is a poor approximation to the smooth trend

-   The spline and polynomial models achieve a good balance: they fit the data nearly as well as the saturated model while using far fewer parameters

-   Both AIC and BIC can guide our choice among these non-nested alternatives, though the final choice should be informed by scientific considerations as well

## Summary comments

-   There are a variety of methods for flexibly modeling continuous covariates

| Approach | R function | Boundary behavior | Interpretable coefficients? | Nested within linear? |
|----------|-----------|-------------------|----------------------------|----------------------|
| Categorized | `factor(cut(...))` | Step function | Yes (group means) | No |
| Polynomial | `poly(x, k)` | Unstable at extremes | No (for degree > 1) | Yes |
| Piecewise linear spline | `lspline(x, knots)` | Linear at tails | Yes (slopes per segment) | Yes |
| B-spline | `bs(x, df)` | Unconstrained | No | No |
| Natural cubic spline | `ns(x, df)` | Linear at tails | No | No |
| Restricted cubic spline | `rcs(x, nk)` | Linear at tails | No | No |
...
-   Spline for estimating means or prediction of new observations
    ...
    -   Be careful of behavior of your splines at the extremes of your continuous covariates
        ...
        -   (Also, don't extrapolate beyond the range of the data)
...
-   Allowed model complexity is, roughly, gain one available degree of freedom per every 10-15 of effective sample size
    ...
    -   These are approximations, and the allowed model complexity can depend on other characteristics of the data too
...
-   Flexible models for confounders
    ...
    -   There is not much to be lost and potentially much to be gained by pre-specifying a flexible model for a continuous confounder
        ...
        -   If it is a straight line, a flexible spline or a linear term can capture the association (either would adjust for confounding)

        -   If it is not a straight line, a flexible spline can capture the association while a linear term cannot (spline would more completely adjust for confounding)

```{r, fig.cap="Linear (straight line) and natural spline (3 d.f.) fits to log of FEV and height.  The relationship is approximately a straight line, which can be captured by either assuming a straight line association or allowing for a flexible fit."}
...
-   The betas for the naturally-splined terms (age, height) are difficult to interpret directly. However, we can test for the overall association of age or height with FEV using the Wald approach by simultaneously testing all spline coefficients for each variable.

-   Test for association between FEV and age, controlling for smoking, height, and sex:

```{r}
linearHypothesis(m1, c("ns(age, 3)1", "ns(age, 3)2", "ns(age, 3)3"),
                 vcov=sandwich(m1))
```

-   Test for association between FEV and height, controlling for smoking, age, and sex:

```{r}
linearHypothesis(m1, c("ns(height, 3)1", "ns(height, 3)2", "ns(height, 3)3"),
                 vcov=sandwich(m1))
```

-   Both tests simultaneously set all three spline coefficients for the variable to zero. This is the appropriate test when using a flexible model — we must test all of the terms derived from a single scientific measurement together, not individually.

-   For the predictor of interest, smoker, we have the usual interpretation in terms of holding other covariates constant. "Among subjects of the same age, height, and sex but differing in smoking status..." However, we may be controlling for the confounding effects of age and height more completely by using flexible spline functions.
