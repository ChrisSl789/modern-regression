...
```{r, fig.cap="Natrual spline (with knots at 3.5 and 9.5) copmare with restricted cubic spline with parms set to give the same fit.  Both spline approaches impose a linearity constraint at the tails.  Fits are same using these inputs."}
ggplot(fev, aes(x=age, y=height)) + geom_point() + theme_bw() + 
...
```{r, fig.cap="Natrual cubic spline (3 d.f) copmare with restricted cubic spline using default inputs.  Both spline approaches impose a linearity constraint at the tails, but with different default knot locations the fits are different."}
ggplot(fev, aes(x=age, y=height)) + geom_point() + theme_bw() + 
  geom_smooth(method=lm, formula=y ~ ns(x, df=3), se=FALSE, aes(color="Blue")) + 
...
```{r, fig.cap="Natrual spline (3 d.f) copmare with restricted cubic spline with knots at 9.5 and 15.5.  Both spline approaches impose a linearity constraint at the tails."}
ggplot(fev, aes(x=age, y=height)) + geom_point() + theme_bw() + 
  geom_smooth(method=lm, formula=y ~ ns(x, df=3), se=FALSE, aes(color="Blue")) + 
...
-   Allowed model complexity is, roughly, 10-15 effective sample size per degree of freedom

    -   These are all approximations, and the allowed sample size can depend on other characteristics of the data too
...
-   Flexible models for a precision variable
    ...
    -   If justified, can consider a spline model too
...
-   Flexible models for confounders
    ...
    -   If we are not interested in interpreting the effect of the confounder, than we are free to model it a variety of different ways
