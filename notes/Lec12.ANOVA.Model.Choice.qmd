...
-   Predicted values
    ...
    -   After fitting a regression model, the `predict` function will provide predicted values for each case
...
-   Testing for the dose effect
    ...
    -   We must use `linearHypothesis` to jointly test the dose coefficients because the model includes the baseline measurement
...
-   To test the treatment effect, we can use the partial t-test from the output directly as we are only testing one parameter
...
-   The partial t-test for the quadratic term can be interpreted as a test for departure from a linear dose response

    -   It is highly significant, suggesting the relationship is not purely linear
...
-   The partial t-test for the `trt` term tests whether there is a jump from placebo to the active dose groups (the threshold effect)

    -   Here, it is highly significantly different from 0, indicating a large initial effect of treatment

-   The partial t-test for the `dose` term tests whether there is an additional linear trend among the active dose groups

    -   It is significantly different from 0 ($p = 0.01$)
...
```{r}
linearHypothesis(m.thres, c("trt", "dose"))
```
...
-   Repeat the process 5000 times (representing 5000 studies)
...
-   Multiple comparison issues
    ...
    -   How many of the 5000 simulated trials had at least one model with a $p$-value $< 0.05$?
...
-   In Lecture 13, we will see how model choice decisions directly impact predictive accuracy; both for continuous outcomes (prediction intervals) and binary outcomes (classification, ROC curves)
