...
```{r}
# Simulate a dataset for the case study
set.seed(123)
n.sim <- 1000
sim.data <- data.frame(
  sex = factor(sample(c("female","male"), n.sim, replace=TRUE)),
  cholesterol = rnorm(n.sim, 200, 40),
  age = round(rnorm(n.sim, 50, 12)),
  blood.pressure = rnorm(n.sim, 130, 20)
)

# Generate binary outcome with known relationships
lp <- with(sim.data, -2 + 0.1*(age-50) - 0.0008*(age-50)^2 +
             0.005*(cholesterol-200)*(sex=="male") + 0.002*rnorm(n.sim))
sim.data$y <- rbinom(n.sim, 1, plogis(lp))

# Fit the model using lrm from rms package
dd <- datadist(sim.data)
options(datadist="dd")

f <- lrm(y ~ sex*rcs(cholesterol) + pol(age,2) + blood.pressure,
         data=sim.data, x=TRUE, y=TRUE)
f
```

-   Validation using bootstrap with backwards stepwise regression

    -   Different bootstrap samples will remove different covariates
...
```{r, cache=TRUE}
v <- validate(f, B=150, bw=TRUE, rule="p", sls=.1, type="individual")
v
```

-   Key output to focus on:

    -   `index.orig`: Discrimination index in the original sample

    -   `index.corrected`: Optimism-corrected index (more honest estimate of performance)

    -   `optimism`: Difference between training and test performance (measure of overfitting)

```{r, fig.cap="Calibration plot for the logistic regression model"}
cal <- calibrate(f, B=150)
plot(cal, main="Bootstrap Calibration")
```

-   The calibration plot shows how well predicted probabilities match observed event rates

    -   A well-calibrated model follows the 45-degree line

    -   Departures from the line indicate systematic over- or under-prediction
