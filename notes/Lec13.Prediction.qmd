...
```{r, fig.height=6, fig.width=12, fig.cap="Stepping through cutpoints: each row shows how a threshold on the risk score (left) maps to a point on the ROC curve (right)"}
cutpoints <- c(0.70, 0.50, 0.30, 0.15, 0.05)

par(mfrow=c(length(cutpoints), 2), mar=c(4, 4, 3, 1))

for(cp in cutpoints) {
  # Compute sensitivity and specificity at this cutpoint
  pred.pos <- fev$risk.score >= cp
  sens <- sum(pred.pos & fev$smoker == 1) / sum(fev$smoker == 1)
  spec <- sum(!pred.pos & fev$smoker == 0) / sum(fev$smoker == 0)
  fpr <- 1 - spec

  # Left panel: Box plots with cutpoint line
  boxplot(risk.score ~ smoker, data=fev, horizontal=FALSE,
  ...
  abline(h=cp, col="red", lwd=2, lty=2)

  # Shade the regions above the cutpoint
  text(0.6, cp + 0.04, sprintf("c = %.2f", cp), col="red", cex=0.9, font=2)

  # Right panel: ROC curve with highlighted point
  plot(1 - roc.demo$specificities, roc.demo$sensitivities,
  ...
  points(fpr, sens, col="red", pch=19, cex=2.5)
  text(fpr + 0.08, sens - 0.06,
       sprintf("(%.2f, %.2f)", fpr, sens), col="red", cex=0.9, font=2)
}
```
...
-   Approach in R
    ...
    -   Use the `pROC` package for ROC curves and AUC comparison
...
```{r}
# Formal test comparing AUC
roc.test(roc.height, roc.age)
```
